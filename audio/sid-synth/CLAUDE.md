...
| `tools/make-birthday-song.js` | Generates `sids/happy-birthday.sng`, the starting tune of the `sid-tracker-birthday` Pages build (see `scripts/build-pages.mjs`) |

Corpus (ALL generated by this repo - no third-party tune is stored here):
`sids/default-song.sng` (the demo song, `tools/make-default-song.js`),
`sids/happy-birthday.sng` (the birthday build's starting tune,
`tools/make-birthday-song.js` - 3/4, 12-row patterns, the only odd-length
patterns in the corpus) + `tests/songs/features.sng` (11 feature-isolating
subtunes, `tests/make-test-songs.js`). Each subtune is checked
worklet-vs-ref, exported-SID-vs-ref and resaved-.sng-vs-ref, plus
ref-vs-golden drift detection - 52 checks total. Golden dumps
(`tests/golden/*.json.gz`) also let verify run on machines without gcc.
...
- GT2's 6502 player stores unmasked set-bytes in PW-hi ($88 vs gplay's $08) —
  SID hardware ignores that nibble's top bits; regdiff masks them by default.
  (It DOES skip gplay's `& 0xfe` on PW-lo, which hardware does not ignore —
  see the pulse-width LSB entry below.)
...
- **Pulse-width LSB**: gplay.c writes `cptr->pulse & 0xfe` (PW-lo bit 0 always
  0); player.s writes the byte raw. Any ODD pulse value therefore makes the
  exported .SID differ from the reference by 1 LSB on $02/$09/$10 for as long
  as the sweep runs. regdiff does NOT mask this, so keep generated PWM
  arithmetic even (`depth/rate` an even integer, `center - depth/2` even) and
  the corpus stays at zero diffs - see the note in tools/make-birthday-song.js.
