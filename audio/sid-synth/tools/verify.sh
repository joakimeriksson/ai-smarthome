...
# Corpus: song-file:subtunes. EVERY song here is our own work - no third-party
# tune is stored in this repo (see .gitignore). features.sng is generated by
# tests/make-test-songs.js (feature-isolating subtunes); default-song.sng is
# generated by tools/make-default-song.js and supplies the dense "real song"
# material the corpus used to get from a third-party .sng: 27 patterns, 6
# instruments, all four tables, arps/drums/PWM/filter/vibrato, order-list
# transposes and 8xy/Axy/Fxy commands. happy-birthday.sng
# (tools/make-birthday-song.js) is the birthday build's starting tune and adds
# a 3/4 song with 12-row patterns - the only odd-length patterns in the corpus,
# which is exactly where a row/tempo off-by-one would hide. All three are
# deterministic, so regenerating them yields identical bytes and the golden
# dumps stay valid.
CORPUS=(
  "sids/default-song.sng:0"
  "sids/happy-birthday.sng:0"
