...
- [x] **Curated public build** (`build_public.py`): `./public` is a standalone static site containing only `sensitivity == "public"` nodes and the edges wholly between them — no editor, no workshop mode, no person-link bar, no write API. Node fields pass an **allowlist**, so provenance and any future internal attribute cannot leak by being forgotten; the build verifies its own output and aborts rather than shipping a leak. *Currently 70 nodes: the external prior-art papers. No stones are public yet, so the public build has no roadmap — that is a curation decision, not a missing feature.*
